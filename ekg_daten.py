...
class EKGAnalyzer:
    ...
    def load_person_data(file_path):
        """
        Lädt die Personendaten aus einer Datei.
        
        :param file_path: Pfad zur Personendatendatei
        :return: Liste der Personendaten
        """
    ...
    def analyze_and_plot_ekg(current_user_name, personen_data):
        ...
        ekg_data = EKGAnalyzer.load_ekg_data(selected_ekg_test['result_link'])
        ekg_signal = ekg_data[0]
        peaks = EKGAnalyzer.detect_peaks(ekg_signal)
        mean_hr, max_hr = EKGAnalyzer.calculate_heart_rate(peaks)
